      ...
      <main className="min-h-screen bg-slate-50">
            ...
            {featured.map((article) => (
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  <span
                    className={`px-2 py-0.5 rounded-full text-xs font-medium ${categoryColors[article.category] || 'bg-slate-100 text-slate-600'
                      }`}
                  >
                    {article.category}
                  </span>
              ...
              {categories.map((cat) => (
                <div key={cat} id={cat.toLowerCase().replace(/\s+/g, '-')}>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span
                      className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${categoryColors[cat] || 'bg-slate-100 text-slate-600'
                        }`}
                    >
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    {grouped[cat].map((article) => (
                      <Link
                        key={article.slug}
                        href={`/guias/${article.slug}`}
                        className="group flex items-start gap-4 bg-white rounded-3xl border border-slate-100 p-5 hover:border-blue-200 hover:shadow-md transition-all duration-300"
                      >
        ...
        <section className="pb-16 lg:pb-20">
          <div className="container mx-auto px-4">
            <div className="relative overflow-hidden bg-gradient-to-br from-blue-600 via-blue-700 to-blue-800 rounded-3xl p-8 md:p-14 text-center text-white shadow-xl ring-1 ring-black/5">
              <div className="absolute -top-24 -right-24 w-72 h-72 bg-white/10 blur-[80px] rounded-full pointer-events-none" />
              <div className="absolute -bottom-24 -left-24 w-72 h-72 bg-white/5 blur-[60px] rounded-full pointer-events-none" />

              <div className="relative z-10">
                <h2 className="text-2xl font-bold md:text-4xl tracking-tight">¿Tenes una consulta técnica?</h2>
                <p className="mt-4 text-blue-50 text-[17px] max-w-xl mx-auto leading-relaxed">
                  Si no encontras la respuesta en nuestras guías, escribinos y te asesoramos personalmente.
                </p>
