...
// ─── Inner shell — uses Clerk hooks (ClerkProvider is already above it) ───────
function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  const navItems = isLoaded
    ...
    : ALL_NAV_ITEMS.filter(item => item.roles.includes('staff'));
    ...
    <div className="min-h-screen bg-slate-100">
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-30 bg-white border-b border-slate-200 h-16 flex items-center px-4 lg:px-8">
          <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 hover:bg-slate-100 rounded-lg mr-4" aria-label="Abrir menú">
          ...
          <div className="flex items-center gap-4">
            ...
            <UserButton afterSignOutUrl="/admin/login" appearance={{ elements: { avatarBox: 'w-9 h-9', userButtonPopoverCard: 'shadow-xl' } }} />
          </div>
        </header>
...
// ─── Outer layout — provides ClerkProvider, then renders the shell ────────────
